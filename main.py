...
async def create_document_for_vehicle(
    vehicle_id: int, 
    ...
    try:
        ...
        crud.create_audit_log(
            ...
            new_value=db_document
        )
        try:
             db.commit()
        except Exception as e:
             db.rollback()
             print(f"寫入 Document Audit Log 失敗: {e}")
        return db_document
    except Exception as e:
...
def update_work_order_api(
    work_order_id: int,
    work_order_update: schemas.WorkOrderUpdate, # 使用 WorkOrderUpdate Schema
    db: Session = Depends(get_db),
    actor_id: int = Depends(get_current_actor_id)
):
    """
    ...
    try:
        ...
        # 重新查詢以確保關聯資料 (vendor, vehicle) 也是最新的
...
# (Note: read_vehicle_violations_api is missing, should be added if needed)
...
def update_reservation_status_api(
    reservation_id: int,
    status_update: schemas.ReservationUpdate,
    db: Session = Depends(get_db),
    actor_id: int = Depends(get_current_actor_id)
):
    try:
